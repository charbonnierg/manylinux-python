...
function build() {
    ...
    docker buildx build \
        ...
        -f ${DOCKERFILE} \
        --push \
        -t "quara/manylinux_2014:${TAG}" \
        .
}

function showHelp() {
    echo -e "Build docker images which can be use to create manylinux_2014 wheels\n"
...
function main() {
    ...
    case "$PLATFORM" in
        ...
        manifest)
            ...
            # Create manifest
            docker manifest create quara/manylinux:2014 \
                quara/manylinux_2014:amd64 \
                quara/manylinux_2014:arm64 \
                quara/manylinux_2014:armv7
            # Push manifest
            docker manifest push quara/manylinux:2014
            ;;
